...
base_path="data/corpora/test_apply_learned_shape"
...
./bpe/mnbpe --log \
    --apply "${bpe_path}/shape_vocab" \
    "$orig_path" "$base_path" \
    | tee "${base_path}/test_apply_learned_shape.log" -a

python3 plot_bpe_log.py \
    "${base_path}" \
    "${base_path}/test_apply_learned_shape.log"

cp "${bpe_path}/shape_vocab" "$base_path"
cp "${bpe_path}/paras" "$base_path"
cp "${bpe_path}/pathlist" "$base_path"
python3 make_arrays.py --bpe --debug \
    --log "${base_path}/test_apply_learned_shape.log" \
