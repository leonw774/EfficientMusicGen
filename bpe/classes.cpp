...
// This decide how shape set should be represented sequentially
...
std::vector<Shape> readShapeVocabFile(std::ifstream& inShapeFile) {
    std::vector<Shape> shapeDict = getDefaultShapeDict();
    inShapeFile.seekg(0, std::ios::beg);
    std::string line;
    while (inShapeFile.good()) {
        std::getline(inShapeFile, line, '\n');
